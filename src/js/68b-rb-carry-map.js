...
function _cmSmooth(pts, xBounds){
  const f=v=>(+v).toFixed(1);
  const bounded=(x,y)=>{
    if(!xBounds) return [x,y];
    const [minX,maxX]=xBounds(y);
    return [Math.max(minX,Math.min(maxX,x)),y];
  };
  if(pts.length<2) return '';
  const start=bounded(pts[0][0],pts[0][1]);
  let d=`M${f(start[0])},${f(start[1])}`;
  for(let i=0;i<pts.length-1;i++){
    const p0=pts[Math.max(0,i-1)], p1=pts[i], p2=pts[i+1], p3=pts[Math.min(pts.length-1,i+2)];
    const c1=bounded(p1[0]+(p2[0]-p0[0])/6, p1[1]+(p2[1]-p0[1])/6);
    const c2=bounded(p2[0]-(p3[0]-p1[0])/6, p2[1]-(p3[1]-p1[1])/6);
    const end=bounded(p2[0],p2[1]);
...
function _cmRunPath(sx, sy, x0, losY, x1, y1, laneW, seed, reachedLine, ob, xBounds){
  ...
  if(reachedLine){
    ...
    if(ob){
      ...
      if(run>40){
        ...
        return _cmSmooth(pts.concat([[x1, y1]]), xBounds).replace(/ C[^C]*$/, '') + ` L${(+x1).toFixed(1)},${(+y1).toFixed(1)}`;
      }
      pts.push([x1, y1]);
      return _cmSmooth(pts, xBounds);
    }
    // Upfield paths use three seeded shapes: direct, one cut, or a second cut/counter.
    // Short gains stay simple; longer runs earn more room to change direction. The last
    // stretch is always straight to the recorded result, where he got away or was tackled.
    const run=losY-y1;
    let x=x0, lastCut=0;
    if(run>45){
      const shape=rnd();
      if(shape>=0.2){
        const firstCut=0.14+rnd()*0.25;
        const wide=shape>=0.62;
        const lean=sgn()*laneW*(wide ? (0.38+rnd()*0.58) : (0.1+rnd()*0.28));
        x+=lean;
        lastCut=firstCut;
        pts.push([x,losY-run*firstCut]);
        if(run>110 && (wide || rnd()<0.24)){
          lastCut=Math.min(0.72,firstCut+0.18+rnd()*0.2);
          x+=rnd()<0.72 ? -lean*(0.45+rnd()*0.75) : lean*(0.18+rnd()*0.38);
          pts.push([x,losY-run*lastCut]);
        }
      }
    }
    // the finish is a straight run: smooth up to the last bend, then a straight line home
    if(run>40){
      const tailAt=lastCut+(1-lastCut)*(0.55+rnd()*0.3);
      const tail=[x1+(x-x1)*(0.08+rnd()*0.22),losY-run*tailAt];
      pts.push(tail);
      return _cmSmooth(pts.concat([[x1, y1]]), xBounds).replace(/ C[^C]*$/, '') + ` L${(+x1).toFixed(1)},${(+y1).toFixed(1)}`;
    }
  } else {
...
function carryMapSVG(plays, title, sub, tag){
  ...
  for(const i of order){
    ...
    const d=_cmRunPath(sx, sy, x0, losY, x1, y1, laneW, seed, endYd>=0, !!ob, y=>[left(y)+3,right(y)-3]);
