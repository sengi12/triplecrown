...
async function pcardFetchDstSeason(season){
  // An in-progress season only has weeks up to the one being played; asking for the rest
  // wastes 12 round trips and (in a frozen build) would leak weeks that "haven't happened".
  // (tcSeasonMaxWeek is the whole season — the slider's track — not the weeks with games.)
  const maxWk = (typeof tcSeasonPlayedWeek==='function') ? tcSeasonPlayedWeek(season)
              : (typeof tcSeasonMaxWeek==='function') ? tcSeasonMaxWeek(season) : 18;
  const ck = `${season}:${maxWk}`;
  if(_dstWeekCache[ck]) return _dstWeekCache[ck];
  if(_dstInFlight[ck]) return _dstInFlight[ck];
  _dstInFlight[ck] = (async()=>{
  const WEEKS = maxWk;
  ...
  if(!failed.length) _dstWeekCache[ck] = byTeam;   // partial seasons are retried next open
  return byTeam;
  })().finally(()=>{ delete _dstInFlight[ck]; });
  return _dstInFlight[ck];
}
...
function _dstSeasons(){
  ...
  const out = [];
  // The season in progress goes first — "how has this defense done so far" is the whole
  // point of opening the card in-season. Completed seasons follow as reference.
  if(typeof hasSeasonStarted==='function' && hasSeasonStarted() &&
     typeof completedWeeks==='function' && completedWeeks()>0 && typeof TC_SEASON!=='undefined'){
    out.push(String(TC_SEASON.year));
  }
  for(let y=proj-1; y>=proj-4; y--){ const v=String(y); if(out.indexOf(v)<0) out.push(v); }
  return out;
}
...
function renderPcardTeamDef(pid){
  ...
  const blocks = seasons.map(season=>
    `<div id="dst_${team}_${season}" class="pcard-season"><div class="pcard-loading">Loading ${(typeof tcSeasonLabel==='function')?tcSeasonLabel(season):season}…</div></div>`
  ).join('');
...
function _dstSeasonBlock(weeks, season, team){
  if(!weeks || !weeks.length) return `<div class="pcard-mini-note">No ${(typeof tcSeasonLabel==='function')?tcSeasonLabel(season):season} defensive weeks yet.</div>`;
  ...
  const full = (typeof teamDisplayName==='function' ? teamDisplayName(team) : team);
  const seasonLbl = (typeof tcSeasonLabel==='function') ? tcSeasonLabel(season) : season;
  const liveTag = (typeof tcIsLiveSeason==='function' && tcIsLiveSeason(season))
    ? ` <span class="pcard-live-tag" title="Season in progress — through the completed weeks">in progress</span>` : '';
  return `<div class="pcard-season-title"><img src="${NFL_LOGO(team)}" class="pcard-dst-logo" onerror="this.style.display='none'"> ${full} D/ST · ${seasonLbl}${liveTag}</div>
