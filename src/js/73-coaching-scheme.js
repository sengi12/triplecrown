...
let schemeBenefactorSort = 'tgt';   // target leaders rank by TARGET share; 'opp' (touches) is the opt-in view
...
function _schemeToGroup(g){
  ...
  return {
    ...
    assigns: assigns.map(a=>({
      ...
      src: (a && a.src) || 'form',   // 'inf' / 'season' on a charted-sets season
    })),
    lanes: lanes,
...
function _schemeBindSwipeClose(host){
  ...
  // Mirrors the player card (attachPcardSwipe): the drag must START on the modal's
  // non-scrolling chrome — header, view tabs, season tabs. Those never scroll, so a
  // downward pull there is unambiguous on every tab. It used to be gated on
  // schemeViewTab==='playbook' instead, which is why the gesture died the moment Red Zone /
  // Regression / Scheme was open: the listeners were alive, they just bailed out.
  // (On the Playbook tab the content is a srcdoc iframe whose touches never reach us, so
  // the chrome was always the only place a swipe could begin anyway.)
  const onChrome = (el)=> !!(el && el.closest && el.closest('.scheme-head, .scheme-view-tabs, .scheme-tabs'));
  const onControl = (el)=> !!(el && el.closest && el.closest('.scheme-close, .scheme-back'));
  let fromChrome = false;
  modal.addEventListener('touchstart', e=>{
    if(!schemeOverlayOpen){ dragging=false; y0=null; x0=null; return; }
    if(e.touches.length!==1){ dragging=false; y0=null; x0=null; return; }
    const t = e.touches && e.touches[0];
    if(!t){ dragging=false; y0=null; x0=null; return; }
    if(onControl(e.target)){ dragging=false; y0=null; x0=null; return; }
    fromChrome = onChrome(e.target);
    // A drag that begins in the scrollable body only counts when the body is at the top —
    // and that is decided NOW, not at touchend, so a rubber-band back to 0 mid-gesture
    // cannot turn a scroll into a close.
    if(!fromChrome && modal.scrollTop > 1){ dragging=false; y0=null; x0=null; return; }
    y0 = t.clientY;
    x0 = t.clientX;
    dy = 0;
    dragging = true;
  ...
  modal.addEventListener('touchmove', e=>{
    if(!schemeOverlayOpen) return;
    if(!dragging || !Number.isFinite(y0) || !Number.isFinite(x0)) return;
    if(!fromChrome && modal.scrollTop > 1) return;
    const t = e.touches && e.touches[0];
    if(!t) return;
    dy = t.clientY - y0;
  ...
  const finish = (e)=>{
    if(!dragging){ return; }
    dragging = false;
    const shouldTrack = schemeOverlayOpen && (fromChrome || modal.scrollTop <= 1);
  ...
  _schemeRzUsagePromise[key] = (async()=>{
    ...
    if(roster.length && typeof fetchPlayerWeekly==='function'){
      ...
      results.forEach(r=>{
        ...
        let rzTgt = 0, rzAtt = 0, recTd = 0, rushTd = 0, tgt = 0, att = 0;
        ...
        if(rzTgt > 0 || rzAtt > 0){
          ...
          meta[p.player_id] = { name: p.name, pos: p.pos, rzTgt, rzAtt, recTd, rushTd, tgt, att };
        }
      });
    }
...
function _schemeRenderActualBenefactors(snap, teamCode, season){
  ...
  const targetSort = (schemeBenefactorSort === 'opp') ? 'opp' : 'tgt';
  // A single red-zone look is not a role. Leaders need a few of them before they rank.
  const MIN_RZ = _SCHEME_MIN_RZ_SAMPLE;
  const targetRows = all.filter(r=>r.rzTgt >= MIN_RZ)
    .sort((a,b)=> targetSort==='tgt'
      ? ((b.tgtShare - a.tgtShare) || (b.oppShare - a.oppShare))
      : ((b.oppShare - a.oppShare) || (b.tgtShare - a.tgtShare)))
    .slice(0, 8);
  const rushRows = all.filter(r=>r.rzAtt >= MIN_RZ)
...
function _schemePersonnelName(g){
  ...
  // A charted set (season in progress): the backfield count is charted, the TE/WR split is
  // the team's most common one from last season — a real personnel code, marked estimated.
  const est = (g && g.pers_assumed) ? ' (est.)' : '';
  if(/^\dB$/.test(p)) return `${_schemeNumber(g && g.backs, 0)}-back sets`;
  if(/^\d{2}$/.test(p)) return `${p} personnel${est}`;
  const b = _schemeNumber(g && g.backs, 0);
  const t = _schemeNumber(g && g.te, 0);
  return `${b}${t} personnel${est}`;
}
function _schemePersonnelDetail(g){
  const est = (g && g.pers_assumed) ? ' (est.)' : '';
  return `${_schemeNumber(g && g.backs,0)}RB ${_schemeNumber(g && g.te,0)}TE ${_schemeNumber(g && g.wr,0)}WR${est}`;
}
...
function _schemeRenderRzTdSource(p){
  ...
  const nRanked = (lg && Array.isArray(lg.rzAir)) ? lg.rzAir.length : nT;
  const rkTxt = r => (r&&nRanked)? `${_schemeOrdinal(r)} of ${nRanked}` : '';
...
function _schemeSchemeProfile(p){
  ...
  const paEpa = hasType('pa') ? pa.epa : null;
  const motionRate = (totalPlays && hasType('motion'))? 100*typeTotal('motion')/totalPlays:null;
  const nohuddleRate = (totalPlays && hasType('nohuddle'))? 100*typeTotal('nohuddle')/totalPlays:null;
  const pers = {};
...
function _schemeRenderScheme(p){
  ...
  // Every rate here is league-ranked: a 58% pass rate means nothing until you know it is
  // 4th or 24th. _schemeSchemeLeague already profiles all 32 teams for the RZ TD card.
  const lg = _schemeSchemeLeague(season);
  const rankOf=(v, arr)=> (lg && Number.isFinite(v) && Array.isArray(arr) && arr.length>1) ? _schemeRankInLeague(v, arr, 'desc') : null;
  const rkHtml=(r, arr)=> r ? `<span class="scheme-insight-rank ${_schemeRankClass(r, arr.length)}">${_schemeOrdinal(r)} of ${arr.length}</span>` : '';
  const rkTxt=(r, arr)=> r ? ` · ${_schemeOrdinal(r)} of ${arr.length}` : '';
  ...
  const persCards = sp.persList.map(r=>{
    ...
    // top five, so a rank against 6 teams ("4th of 6" for 20 personnel) says nothing.
    // Rank only the packages most of the league actually runs.
    const arr = lg && lg.persShare && lg.persShare[r.key];
    const rk = (Array.isArray(arr) && arr.length >= 20) ? rankOf(r.share, arr) : null;
    return `<div class="scheme-op-card"><div class="scheme-op-k">${_schemeEscHtml(r.key)}</div><div class="scheme-op-v">${tag(`${r.share.toFixed(0)}% ${rkHtml(rk, arr||[])} <span class="scheme-insight-rank neutral">${pct(r.passRate)} pass</span>`, `${r.key} usage`, `${r.share.toFixed(1)}% of snaps${rkTxt(rk, arr||[])} · ${pct(r.passRate)} pass · ${epaTxt(r.epa)} EPA`, 'personnel_share')}</div></div>`;
  }).join('');
...
// League red-zone target propensity per snap on the field, by position (nflverse
// 2021-2024, inside the 20): a WR on the field draws ~1.0 RZ targets per 4 pass plays,
// a TE ~0.8 of that, a back ~0.55. Used ONLY by the modeled fallback when no actual
// Sleeper red-zone logs exist for the season.
const _SCHEME_RZ_TGT_W = { WR:1.0, TE:0.8, RB:0.55, FB:0.3 };

// League touchdown rates per opportunity (nflfastR 2021-2024, regular season):
//   inside-the-20 target  → TD  ≈ 19%      outside → ≈ 2%     (~830 pass TD, ~60% from the RZ)
//   inside-the-20 carry   → TD  ≈ 11%      outside → ≈ 1%     (~500 rush TD, ~75% from the RZ)
// These are the baseline every player is held to. The previous model held each player
// to HIS OWN TEAM's TD-per-red-zone-touch rate and charged every touchdown, including
// 60-yarders, against red-zone touches only — a zero-sum split that tagged deep threats
// "sell" and goal-line backs "buy" regardless of how either was actually converting.
const _SCHEME_TD_RATES = {
  rzTgt: { WR:0.19, TE:0.21, RB:0.13, QB:0.19, FB:0.16, default:0.18 },
  oTgt:  { WR:0.022, TE:0.018, RB:0.012, QB:0.015, FB:0.010, default:0.020 },
  rzAtt: { RB:0.11, WR:0.13, TE:0.13, QB:0.18, FB:0.15, default:0.11 },
  oAtt:  { RB:0.011, WR:0.012, TE:0.010, QB:0.012, FB:0.008, default:0.011 },
};
function _schemeTdRate(kind, pos){
  const t = _SCHEME_TD_RATES[kind] || {};
  return t[String(pos||'').toUpperCase()] != null ? t[String(pos||'').toUpperCase()] : t.default;
}
function _schemeExpectedTd(m){
  const pos = m.pos;
  const rzTgt = m.rzTgt||0, rzAtt = m.rzAtt||0;
  const oTgt = Math.max(0, (m.tgt||0) - rzTgt);
  const oAtt = Math.max(0, (m.att||0) - rzAtt);
  return rzTgt*_schemeTdRate('rzTgt',pos) + oTgt*_schemeTdRate('oTgt',pos)
       + rzAtt*_schemeTdRate('rzAtt',pos) + oAtt*_schemeTdRate('oAtt',pos);
}

// TDs over/under league expectation before a player is flagged either way.
const _SCHEME_REG_FLAG = 1.5;
...
function _schemeBuildRegression(snap){
  ...
  Object.keys(meta).forEach(pid=>{
    ...
    if(opp<=0) return;
    const exp=_schemeExpectedTd(m);
    teamOpp+=opp; teamTD+=td; teamExp+=exp;
    rows.push({ player_id:pid, name:m.name||pid, pos:m.pos||'', rzTgt:m.rzTgt||0, rzAtt:m.rzAtt||0,
      tgt:m.tgt||0, att:m.att||0, opp, td, exp, delta: td-exp });
  });
  // Kept for the header line; no longer the baseline any player is judged against.
  const rate = teamOpp>0? teamTD/teamOpp : 0;
  return { rows, rate, teamOpp, teamTD, teamExp };
}
...
function _schemeRenderRegression(p){
  ...
  const tip=_schemeInfoTip('TD regression', "Compares each player's actual season touchdowns to the league-expected total for his workload: red-zone targets and carries at league inside-the-20 conversion rates (~19% per target, ~11% per carry) plus every other target and carry at the league open-field rates (~2% / ~1%). Judged against the league, not this offense, and against ALL of his touches — so a deep threat is not penalised for scoring from distance, and a goal-line back is not excused for converting at the league norm. Under-scorers (1.5+ TDs short) are positive-regression buy-low candidates; over-scorers (1.5+ over) are TD-dependent sell-high risks.");
  const rowHtml=(r,i)=>{
    ...
    const flag = r.delta>=_SCHEME_REG_FLAG?'sell':(r.delta<=-_SCHEME_REG_FLAG?'buy':'neutral');
    ...
    const chips=`<span class="scheme-benefit-splits">
      ...
      <span class="scheme-benefit-split">${noteWrapHtml(`<b>xTD </b><span>${r.exp.toFixed(1)}</span>`, cm('Expected TDs',`${r.exp.toFixed(1)} league-expected TDs from ${r.rzTgt} RZ tgt + ${r.rzAtt} RZ carr + ${Math.max(0,r.tgt-r.rzTgt)} other tgt + ${Math.max(0,r.att-r.rzAtt)} other carr`,'reg_xtd'),'note-tag-hit')}</span>
    </span>`;
    const headClick=click?`role="button" tabindex="0" title="Open player card" onclick="event.stopPropagation();${click}" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();event.stopPropagation();${click}}"`:'';
    ...
    const reason= flag==='buy'?`${r.td} TD vs ${r.exp.toFixed(1)} league-expected (${r.opp} RZ touches, ${r.tgt+r.att} total) — ${Math.abs(r.delta).toFixed(1)} short. Positive-regression buy-low.`
      : flag==='sell'?`${r.td} TD vs ${r.exp.toFixed(1)} league-expected (${r.opp} RZ touches, ${r.tgt+r.att} total) — ${r.delta.toFixed(1)} over. TD-dependent, regression risk.`
      : `${r.td} TD vs ${r.exp.toFixed(1)} league-expected — in line with workload.`;
    return `<div class="scheme-benefit-row">
      <span class="scheme-benefit-rank">${i+1}</span>
  ...
  const sells=reg.rows.filter(r=>r.opp>=4 && r.delta>=_SCHEME_REG_FLAG).sort((a,b)=>b.delta-a.delta).slice(0,6);
...
function _schemeBuildBenefactors(p, d){
  ...
  downs.forEach(down=>{
    ...
    groups.forEach(g=>{
      ...
      // ONE pool of passPlays split across everyone on the field. The previous version
      // handed a full passPlays to EACH position bucket (WR, TE, RB), so three WRs shared
      // one pool while the lone TE and lone RB each owned a whole one — the goal-line back
      // topped every team's "target" leaders at ~33% opportunity. The route list cannot
      // rank players either (each player's routes sum to 100% by construction), so the
      // weight is the position's league red-zone target propensity per snap instead.
      const onField = eligible.map(a=>{
        const slot = String((a && a.slot) || '');
        const pos = _schemePosFromSlot(slot);
        const charted = Array.isArray(a && a.routes) && a.routes.length > 0;
        return { slot, pos, w: (_SCHEME_RZ_TGT_W[pos] || 0.5) * (charted ? 1 : 0.6) };
      });
      const wSum = onField.reduce((t,x)=>t+x.w, 0);
      if(wSum > 0){
        onField.forEach(x=>{
          const shareOpp = passPlays * (x.w / wSum);
          const rec = (slotDownOpp[x.slot] = slotDownOpp[x.slot] || { pos:x.pos, total:0, d1:0, d2:0, d3:0, d4:0 });
          rec.total += shareOpp;
          rec[`d${down}`] += shareOpp;
        });
      }
...
function _schemeBuildRushBenefactors(p, d){
  ...
  // Down pills are a share WITHIN that down, the same definition the target list uses —
  // the two lists render through identical pills, so they must mean the same thing.
  const runDownTotals = {};
  ['d1','d2','d3','d4'].forEach(k=>{ runDownTotals[k] = Object.values(slotRunOpp).reduce((t,r)=>t+_schemeNumber(r[k],0),0) || 1; });
  const rows = Object.keys(slotRunOpp).map(slot=>{
    ...
    const d1 = 100 * _schemeNumber(rec.d1, 0) / runDownTotals.d1;
    const d2 = 100 * _schemeNumber(rec.d2, 0) / runDownTotals.d2;
    const d3 = 100 * _schemeNumber(rec.d3, 0) / runDownTotals.d3;
    const d4 = 100 * _schemeNumber(rec.d4, 0) / runDownTotals.d4;
...
function _schemeRenderTemplate(template, p){
  ...
  return template
    ...
    .replace(/\b20\d{2}\s+·\s+Routes mapped to players/, (p && p.data && p.data.charting_only) ? `${season} · Charted sets · routes estimated` : `${season} · Routes mapped to players`)
