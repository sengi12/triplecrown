...
function laActivePane(){
  ...
  return (t==='matchup'||t==='lineup'||t==='dvp'||t==='oline'||t==='trends'||t==='chop'||t==='standings') ? t : null;
}
...
function laSeasonView(s, paneOverride){
  ...
  const panes=[chopped?['chop','Chop','axe']:['matchup','Matchup','versus']]
    ...
    .concat([['lineup','Lineup','clipboard'],['dvp','Defense','shield'],['oline','O-Line','wall'],['trends','Trends','chart']]);
...
// ── O-line form: run blocking and pass protection, a recent stretch vs the season ──
// Both windows come from the same recompute the team card's week slider uses
// (_advComputeOlRangeTables in 74-team-tabs, over the in-season ol_weekly block), so
// "last 3" and "season" are the same method on different weeks. Run = the rushing fan's
// rank-average run-blocking score; pass = the windowed Pass Score. Ranked among the lines
// that played in the window.
function laSetOlWin(n){ laState.olWin=Math.max(1, Math.min(8, Number(n)||3)); laRerenderKeepScroll(); }
function _laOlForm(win){
  if(typeof _advComputeOlRangeTables!=='function' || typeof _rbRunScoreAndRankFromTable!=='function') return null;
  const season=String(TC_SEASON.year);
  const wk=(typeof completedWeeks==='function')?completedWeeks():0;
  if(!wk) return null;
  const all=_advComputeOlRangeTables(season, 1, wk);
  if(!all || !all.runTbl || !all.passTbl) return null;
  const lo=Math.max(1, wk-win+1);
  const rec=(lo>1) ? _advComputeOlRangeTables(season, lo, wk) : all;
  if(!rec || !rec.runTbl) return null;
  const passOf=(tbl,tm)=>{ const r=tbl.passTbl.teams[tm]||{}; const v=r.values&&r.values['Pass Score'], k=r.ranks&&r.ranks['Pass Score'];
    return (v!=null && Number.isFinite(Number(v))) ? {score:Number(v), rank:(k!=null?Number(k):null)} : {score:null, rank:null}; };
  const out=[];
  for(const tm in all.runTbl.teams){
    const runA=_rbRunScoreAndRankFromTable(all.runTbl, tm), runR=_rbRunScoreAndRankFromTable(rec.runTbl, tm);
    const passA=passOf(all, tm), passR=passOf(rec, tm);
    if(runA.score==null && passA.score==null) continue;
    out.push({tm, lo, hi:wk, runA, runR, passA, passR,
      dRunRk:(runA.rank!=null && runR.rank!=null) ? runA.rank-runR.rank : null,     // + = climbed
      dPassRk:(passA.rank!=null && passR.rank!=null) ? passA.rank-passR.rank : null});
  }
  return out.length ? out : null;
}
// The Trends pane's two-column board, as a shared helper (the O-Line pane draws the same card).
function _laTwoCols(title,sub,upLbl,upRows,dnLbl,dnRows){
  return `
    <div class="la-ins-bar"><span class="la-ins-lbl">${title}</span><span class="la-ins-sub">${sub}</span></div>
    <div class="card la-trnd-card"><div class="la-trnd-cols">
      <div><div class="la-trnd-h la-trnd-up">${upLbl}</div>${upRows||'<div class="la-ins-sub">nobody in this scope yet</div>'}</div>
...
// ── The O-Line pane: every line's run blocking and pass protection, a stretch vs the season ──
function laOlineView(s){
  const wk=(typeof completedWeeks==='function')?completedWeeks():0;
  ...
  return boards + `<div class="la-note la-note-min">${(typeof tcInfoBtn==='function')?tcInfoBtn('laoline','Reading these boards'):''}</div>`;
}
function _laOlFormBoards(wk){
  const two=_laTwoCols;
  const win=laState.olWin||3;
  const F=_laOlForm(win);
  if(!F) return '';
  const sel=`<span class="la-ol-win">${[2,3,4,6].map(n=>`<button class="pos-filter-btn ${win===n?'active':''}" onclick="laSetOlWin(${n})">Last ${n}</button>`).join('')}</span>`;
  const fmt=(x)=> x==null?'—':x.toFixed(1);
  const rk=(r)=> r==null?'':`#${r}`;
  const thin = wk<win;
  // _laOlForm clamps its "recent" window to start at week 1 once the stretch reaches back
  // that far — at that point "recent" IS "season" (same table twice), every dRunRk/dPassRk
  // is 0, and the surging/slipping filters below come up empty no matter how many weeks are
  // in: show the plain best-to-worst ranking instead, same as the true week-1 case.
  const lo0 = F[0] ? F[0].lo : 1;
  if(wk<2 || lo0<=1){
    // One week: nothing to compare against yet — the season so far, best to worst.
    const byRun=[...F].filter(t=>t.runA.score!=null).sort((a,b)=>b.runA.score-a.runA.score);
    const byPass=[...F].filter(t=>t.passA.score!=null).sort((a,b)=>b.passA.score-a.passA.score);
    const runRow=(t,i)=>_laTeamRow(t.tm, `run blocking ${fmt(t.runA.score)} thru wk ${wk}`, _laVerdict(rk(t.runA.rank), 'of 32', t.runA.rank<=10?'la-trnd-up':(t.runA.rank>=23?'la-trnd-dn':'')), t.runA.rank<=10?'pace-ahead':(t.runA.rank>=23?'pace-behind':''), i+1);
    const passRow=(t,i)=>_laTeamRow(t.tm, `pass protection ${fmt(t.passA.score)} thru wk ${wk}`, _laVerdict(rk(t.passA.rank), 'of 32', t.passA.rank<=10?'la-trnd-up':(t.passA.rank>=23?'la-trnd-dn':'')), t.passA.rank<=10?'pace-ahead':(t.passA.rank>=23?'pace-behind':''), i+1);
    const sub = wk<2 ? `the season so far — form against the season arrives at week 2 ${sel}`
      : `the season so far — pick a shorter stretch than ${win} to see form against the season ${sel}`;
    return two(`O-LINE · RUN BLOCKING · THRU WK ${wk}`, sub,
        'BEST', byRun.slice(0,10).map(runRow).join(''), 'WORST', byRun.slice().reverse().slice(0,10).map(runRow).join(''))
      + two(`O-LINE · PASS PROTECTION · THRU WK ${wk}`, 'the season so far (pass score: pressure, hits, hurries, sacks, pocket time)',
        'BEST', byPass.slice(0,10).map(passRow).join(''), 'WORST', byPass.slice().reverse().slice(0,10).map(passRow).join(''));
  }
  const byRun=[...F].filter(t=>t.dRunRk!=null).sort((a,b)=>b.dRunRk-a.dRunRk || (b.runR.score-a.runR.score));
  const byPass=[...F].filter(t=>t.dPassRk!=null).sort((a,b)=>b.dPassRk-a.dPassRk || (b.passR.score-a.passR.score));
  const move=(d)=> d>0?`▲ ${d}`:(d<0?`▼ ${Math.abs(d)}`:'—');
  const runRow=(t,i)=>_laTeamRow(t.tm, `${fmt(t.runA.score)} (${rk(t.runA.rank)}) season → <b>${fmt(t.runR.score)} (${rk(t.runR.rank)})</b>`,
    _laVerdict(move(t.dRunRk), t.dRunRk>0?'spots up':(t.dRunRk<0?'spots down':'no move'), t.dRunRk>0?'la-trnd-up':(t.dRunRk<0?'la-trnd-dn':'')), t.dRunRk>=4?'pace-ahead':(t.dRunRk<=-4?'pace-behind':''), i+1);
  const passRow=(t,i)=>_laTeamRow(t.tm, `${fmt(t.passA.score)} (${rk(t.passA.rank)}) season → <b>${fmt(t.passR.score)} (${rk(t.passR.rank)})</b>`,
    _laVerdict(move(t.dPassRk), t.dPassRk>0?'spots up':(t.dPassRk<0?'spots down':'no move'), t.dPassRk>0?'la-trnd-up':(t.dPassRk<0?'la-trnd-dn':'')), t.dPassRk>=4?'pace-ahead':(t.dPassRk<=-4?'pace-behind':''), i+1);
  const span=F[0] ? `wks ${F[0].lo}–${F[0].hi}` : '';
  return two(`O-LINE · RUN BLOCKING · LAST ${win}${thin?` (only ${wk} played)`:''}`, `${span} vs the season: who is opening holes right now ${sel}`,
      '▲ SURGING', byRun.filter(t=>t.dRunRk>0).slice(0,10).map(runRow).join(''), '▼ SLIPPING', byRun.filter(t=>t.dRunRk<0).reverse().slice(0,10).map(runRow).join(''))
    + two(`O-LINE · PASS PROTECTION · LAST ${win}${thin?` (only ${wk} played)`:''}`, `${span} vs the season: pass score (pressure, hits, hurries, sacks, pocket time)`,
      '▲ SURGING', byPass.filter(t=>t.dPassRk>0).slice(0,10).map(passRow).join(''), '▼ SLIPPING', byPass.filter(t=>t.dPassRk<0).reverse().slice(0,10).map(passRow).join(''));
}
...
if(typeof TC_INFO_BOOK!=='undefined'){
  ...
  TC_INFO_BOOK.laoline={title:'O-line form', body:`
    Every offensive line over a recent stretch (the Last 2 / 3 / 4 / 6 chips) against its
    season, both from the same week-window recompute the team card's Advanced slider uses.
    <b>Run blocking</b> is the rushing fan's score: the line's league rank on stuff rate,
    explosive rate, yards per rush, yards before and after contact, first-down rate, broken
    tackles, rush yards over expected, box counts and time to the line, averaged.
    <b>Pass protection</b> is the windowed pass score: pressure, hit, hurry and sack rates,
    non-QB sacks, no-blitz pressure and pocket time. Rows read season score (rank) → stretch
    score (rank); the move on the right is rank spots gained or lost. Fields PFR has not
    published yet are left out, never counted as zero. Under two played weeks the boards
    show the season so far, best to worst.`};
