...
function _advComputeGeneralRangeTables(season, lo, hi){
  ...
  for(const tm in pack.teams){
    ...
    teams[tm]={
      offense:{
        ...
        'Rush Success Rate': _advNum((sum.off_run_success!=null && (sum.off_run_plays||0)>0) ? (sum.off_run_success/sum.off_run_plays)*100 : null, 1),
        'Pass Success Rate': _advNum((sum.off_pass_success!=null && (sum.off_pass_plays||0)>0) ? (sum.off_pass_success/sum.off_pass_plays)*100 : null, 1),
      },
      defense:{
  ...
  const spec={
    offense:{lower:new Set(), cols:['EPA/Play','EPA/Pass','EPA/Rush','Points Scored','Yards Per Play','Points Per Drive','Explosive Play Rate','Down Conversion Rate','Rush Success Rate','Pass Success Rate']},
...
// better than it may really be for that week. PFR charts per game/team, not the whole slate
// at once — a `team` arg checks just that row (the card); omitted, it checks the whole
// league (the league table), where even one uncharted team makes cross-team ranks unreliable.
function _advOlPfrLagInfo(season, which, team){
  const pack=(NFLVERSE && NFLVERSE[String(season)] && NFLVERSE[String(season)].ol_weekly) || null;
  ...
  const rowFor=(tm)=>{
    const node=pack.teams[tm]; if(!node) return null;
    return (which==='pass' ? node.pass : node.run)||null;
  };
  const isCharted=(tm, ci)=>{ const row=(rowFor(tm)||[])[latestI]; return !!(row && row[ci]!=null); };
  const missing=[];
  for(const field in lagMap){
    const ci=idx[field]; if(ci==null) continue;
    const gap = team ? !isCharted(team, ci) : Object.keys(pack.teams).some(tm=>!isCharted(tm, ci));
    if(gap) missing.push(lagMap[field]);
  }
  return missing.length ? {week:latestWk, cols:missing} : null;
}
function _advDlPfrLagInfo(season, team){
  const pack=(NFLVERSE && NFLVERSE[String(season)] && NFLVERSE[String(season)].adv_weekly) || null;
  ...
  const latestI=pack.weeks.length-1;
  const latestWk=Number(pack.weeks[latestI]);
  const isCharted=(tm)=>{ const row=(pack.teams[tm]||[])[latestI]; return !!(row && Number(row[ci]||0)>0); };
  const gap = team ? !isCharted(team) : Object.keys(pack.teams).some(tm=>!isCharted(tm));
  return gap ? {week:latestWk, cols:['Pressure Rate','Missed Tackles']} : null;
}
...
// `team` omitted = league-wide check (used by the league table).
function _advPfrLagInfoBtn(tableKey, season, team){
  let info=null, lbl=null;
  if(tableKey==='offensive_line_pass'){ info=_advOlPfrLagInfo(season,'pass',team); lbl='pass-blocking'; }
  else if(tableKey==='offensive_line_run'){ info=_advOlPfrLagInfo(season,'run',team); lbl='run-blocking'; }
  else if(tableKey==='defensive_line'){ info=_advDlPfrLagInfo(season,team); lbl='defensive'; }
  if(!info) return '';
...
function advTrendFor(team, tableKey, col, excludeYear){
  ...
  if(excludeYear!=null && typeof _advComputeGeneralRangeTables==='function'
     && typeof completedWeeks==='function' && completedWeeks()>0){
...
function renderTeamAdvanced(team){
  ...
  const cardFor=(key, srcTeam)=>{
    ...
    const liveOlSeason = (typeof tcIsLiveSeason==='function') && tcIsLiveSeason(advTeamSeason())
      && (typeof completedWeeks==='function') && completedWeeks()>0;
    ...
    const lagBtn = _advPfrLagInfoBtn(key, advTeamSeason(), useTeam);
