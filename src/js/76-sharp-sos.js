...
function nflverseSharpTables(){
  ...
  for(const k in t){
    ...
    const rawCols=(t[k].columns||[]).filter(c=>!HIDE_LAST5.has(c)).map(c=>metricName(c,isDefense));
    const preferred=ORDER[isDefense?'defense':'offense']||[];
    const cols=[...preferred.filter(c=>rawCols.includes(c)), ...rawCols.filter(c=>!preferred.includes(c))];
    ...
    if(teams && typeof teams==='object'){
      ...
      if(firstTeam && !Array.isArray(firstTeam) && firstTeam.values && firstTeam.ranks){
        const lowerCols=isDefense ? ['EPA/Pass Allowed','EPA/Rush Allowed','Points Allowed'] : [];
        ...
        if(!isDefense){
          ['EPA/Pass','EPA/Rush','Points Scored'].forEach(col=>{
            const entries=Object.keys(teams).map(tm=>({tm,v:teams[tm]&&teams[tm].values&&Number(teams[tm].values[col])}))
