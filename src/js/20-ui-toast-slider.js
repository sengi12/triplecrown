...
function tcSliderScaleMax(cur, base, staticMax, floorCap){
  // The cap keys off the BASELINE, not the live value — otherwise dragging a stat up and
  // then re-rendering (an undo, a tab hop) recomputed a wider track and every thumb slid
  // sideways with its value unchanged, which read as a phantom undo. The track only grows
  // when the value actually exceeds it, and holds still otherwise.
  const b=Number(base)||0, c=Number(cur)||0;
  let cap=Math.max(Number(floorCap)||0, tcNiceCeil(b*1.35), staticMax*0.12);
  if(c>cap) cap=tcNiceCeil(c*1.25);
