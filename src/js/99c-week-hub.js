...
function hubAnalyzeLeague(lg, rosters, users, matchups, ctxBase){
  ...
  // waivers: unrostered, skill positions, this week's projection under this scoring
  ...
  const cands = [];
  byId.forEach(row=>{
    if(rostered.has(String(row.player_id)) || !['QB','RB','WR','TE'].includes(row.pos)) return;
    const wp = hubWeekProj(row, ctxBase);
    const perGame = wp.bye||wp.out ? (wp.base||0) : (wp.adj/(wp.defMult||1));
    if(!(perGame>0.5)) return;
    const reasons = hubWaiverReasons(row, wp, ctxBase, teammatesByTeam[String(row.team||'').toUpperCase()]);
    const ros = perGame * weeksLeft;
    const score = ros * (1 + 0.25*reasons.reduce((a,r)=>a+r.w,0));
    cands.push({id:String(row.player_id), name:row.name, pos:row.pos, team:row.team, wp, perGame, ros, reasons, score});
  });
  cands.sort((a,b)=>b.score-a.score);
  const adds = cands.slice(0, HUB_TOP_ADDS);
  // starts over: the weakest starter in MY optimal lineup he could replace
  if(lineup){
    adds.forEach(c=>{
      const targets = lineup.optimal.filter(f=>f.player && hubEligible({pos:c.pos}, f.slot));
      const empty = lineup.optimal.find(f=>!f.player && hubEligible({pos:c.pos}, f.slot));
      if(empty){ c.startsOver={name:'(empty slot)', slot:empty.slot, delta:+c.wp.adj.toFixed(1)}; return; }
      if(!targets.length) return;
      const weak = targets.reduce((m,f)=>(!m || f.player.value<m.player.value) ? f : m, null);
      if(c.wp.adj > weak.player.value) c.startsOver={name:weak.player.name, slot:weak.slot, delta:+(c.wp.adj-weak.player.value).toFixed(1)};
    });
  }
  // drops: bench outside the optimal lineup, weakest first
  let drops = [];
  if(lineup){
    const optIds = new Set(lineup.optimal.filter(f=>f.player).map(f=>f.player.id));
    // everyone outside the optimal lineup — a starter the swap benches included
    const seenD = new Set();
    const outside = Object.values(lineup.current).concat(lineup.bench).filter(b=>!optIds.has(b.id) && !seenD.has(b.id) && seenD.add(b.id));
    drops = outside.map(b=>{
      const reasons=[];
      if(b.unavailable) reasons.push(b.unavailable);
      if(b.rank!=null) reasons.push(`${b.pos}${b.rank} projected`);
      const targets = lineup.optimal.filter(f=>f.player && hubEligible(b, f.slot));
      if(targets.length){ const weak=targets.reduce((m,f)=>(!m || f.player.value<m.player.value)?f:m,null); reasons.push(`${(weak.player.value-b.value).toFixed(1)} behind ${weak.player.name}`); }
      return {p:b, reasons};
    }).sort((a,b)=>a.p.value-b.p.value).slice(0, HUB_TOP_DROPS);
  }
  // FAAB
...
function _hubLeagueCard(res){
  ...
  const adds = res.adds.slice(0, open?HUB_TOP_ADDS:3).map(c=>`<div class="hub-row">
      ...
      <div class="hub-body">${_hubPlayer({id:c.id,name:c.name,pos:c.pos,team:c.team,value:c.wp.adj,wp:c.wp,unavailable:c.wp.bye?'BYE':''},true)}
        <div class="hub-whys">${c.reasons.map(_hubReason).join('')}${c.startsOver?`<span class="hub-why hub-why-starts" title="Beats the weakest starter he could replace in your optimal lineup">starts over ${escHtml(c.startsOver.name)} (${escHtml(c.startsOver.slot)}) +${c.startsOver.delta}</span>`:''}</div></div>
...
<p><b>Adds.</b> Unrostered players ranked by rest-of-season value, with the reason: <b>role opened</b> (a teammate ahead of him is out), <b>usage spike</b> (touches far above his own baseline), <b>soft schedule</b> (next three opponents generous to his position), <b>efficient</b> (his per-target or per-carry rank backs the volume). "Starts over" says he'd beat your weakest starter now.</p>
