...
- **Leaders + Game Center (in season).** On desktop the right sidebar ranks the week's top fantasy finishes under your loaded scoring, and grows into a Game Center: every game in kickoff order with live scores, and for the picked game each side's players by position with the league's exact points, stat lines and fantasy owners. On a phone the same Game Center takes the draft follow's corner instead — a **Games** pill bottom-right (red and glowing while games are on) opens a bottom sheet: half is the week's games as a swipeable rail, pull up for every position group. While the selected team is playing, its score and clock ride in the team-picker bar; tap it to jump to that game. Nothing on the page moves.
...
- **In-season suite (appears automatically once the NFL season kicks off).** The app tracks Sleeper's NFL state (season, phase, week), and when the regular season starts two things happen. The projections view gains a **Live** toggle — the current season's actual stats to date as a read-only reference season (red-zone and air-yard columns, injury designations, and an opt-in **Δ proj** column showing each player's pace against the projections frozen at kickoff, so you can edit all season without moving your own goalposts). And the League Analyzer gains one **Season** tab holding five panes behind Sleeper-style icon chips: **Matchup** (live weekly scoreboard — Sleeper and ESPN leagues alike — your matchup featured, auto-refreshing during games), **Lineup** (optimal lineup vs your set starters, blending your projections with season and recent form, adjusted for opponent defense-vs-position and injuries — and the Waivers tab gains a **This week** lens: the unrostered pool ranked by that same week-adjusted projection, flagging any pickup who would beat the weakest starter he could displace in your optimal lineup), **Defense** (each defense's fantasy points allowed per game vs QB/RB/WR/TE, scored under *your league's* settings), **O-Line** (every offensive line's run blocking and pass protection over a recent stretch against the season — who is opening holes and keeping the pocket clean right now), and **Trends** — Sleeper-style ranked boards for weekly risers/fallers, pace vs projection, raw opportunity, TD-regression candidates built on real red-zone volume, **team tendencies** (who's passing more lately, whose offense is heating up or going cold by EPA), and a **rest-of-season outlook** that re-projects every player from form, remaining schedule and injury status. Advanced weekly data ships in a small in-season sidecar rebuilt weekly by CI from nflverse and retired automatically each offseason.
...
- **Passing Charts.** Every QB has strengths and weakness and it can difficult to determine what those are when just looking at raw fantasy totals at the end of each week. Introducing Passing Charts attached to every QB's player card where you can visually see how a QB performs doing what matters most for fantasy and see beyond the fantasy point totals. The chart opens on the **pass map** of the latest game: every located attempt drawn at its depth and side, the receiver on hover, a green tail for the yards after the catch and a ring + TD tag on every score. The game picker lists every game (playoff rounds included, as WC / DIV / CONF / SB), and "Season" turns the map into the **season summary** — a bubble per spot (side × five-yard depth band) sized by attempts and coloured by completion rate, scores ringed — because a season of throws drawn one by one is a hairball. The zone matrix (passer rating, yards and TDs by zone against the league) a tap away, and the same one-tap link to the passer's Next Gen Stats charts.
...
- **Rushing Fans + Carry Maps.** The Rushing Fan tab opens on the **carry map** of the latest game: every carry drawn from the backfield through the gap he hit and up the field as far as the run went, NGS-style — red for lost yards, gold for 0–4, green for 5+, a white cap on a first down, a ring + TD tag on a score, a red ring + FUM on a fumble lost, and a run that went out of bounds angles out to the sideline it left by (play-by-play says so, and which side) — with the fan a tap away. "Season" bins the carries by gap and distance (a bubble per spot, sized by carries), and the picker lists playoff games after week 18 and the same one-tap link to the rusher's Next Gen Stats charts. The paths are drawn, not tracked: the gap and the end of the run are play-by-play, the bends are a seeded look. A Major aspect of fantasy football, and the sport itself, that is often overlooked is the offensive line. Although available data for individual OL performance is poor, I created an ever evolving +/- algorithm that hands out rush and pass block grades to every qualifying offensive linemen so that you can see clearly how offensive line talent, performance, entanglement, and health directly affects key areas of the run and pass game with every rusher's rushing fan chart!
...
in-app modal hands out exactly this URL — one link, every format). The local stdio server (`tools/tc_mcp.py`) speaks
the same argument — one configured server, any league — building each requested format's board
once and keeping it.

Then, per client (in the app, **☰ → Ask Claude** hands out the one every-format URL with a copy
button and the steps behind ⓘ; the ⚖ setup screen links there too):
...
Open work is tracked in [TODO.md](TODO.md); every merged change is listed in
[CHANGELOG.md](CHANGELOG.md) (one line per pull request, newest first).
