...
chk(/\.pcard\{[^}]*flex-direction:column;\s*max-height:calc\(100vh - 48px\)/.test(html),'card is height-capped flex column');
...
chk(/\.pcard\{[^}]*margin:auto/.test(html),'card centers itself in the overlay (margin:auto, no double-scroll)');
