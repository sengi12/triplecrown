...
run_js_test test_ol_college     "A rookie lineman's OL tab: his college unit's line by season with FBS percentiles — alone without NFL grades, beneath them with; the prospect panel stays out of the way"
...
run_js_test test_played_weeks   "Week sliders on the season in progress: the whole season on the track, the thumbs held to the weeks a team has played (the week in progress once its game kicked off), the rest dimmed; the banner chip reads the played week"
...
run_js_test test_la_myteam_cards "League Analyzer · My Team cards: upgrade path (weakest slot + partners with a spare), trade fit (two-way partners only), league heat map, bye exposure from the sidecar, dynasty-only age timeline, roster construction, the this-week strip, masonry layout"
...
run_js_test test_live_feed      "Live feed: every game at once from the scoreboard's last play, filtered to your synced leagues with the points each play moved under that league's scoring, narrowable to your own matchup"
...
run_js_test test_seed_names      "Seed names: a Sleeper name finds the nflverse key (Joshua/Josh Palmer), exact first, then alias, then the lone teammate; ambiguous or unknown stays plain"
...
run_js_test test_la_trade_history "Trade history: Sleeper's transaction log read into a ledger (adds credit the receiver, drops are the mirror, picks follow owner_id, FAAB follows receiver), filters by player/position/picks/manager/season, the per-leg sweep with its cache and backoff, and earlier seasons resolved against their OWN roster map"
...
run_js_test test_pcard_topbar    "Player card · top bar: icon tabs (inactive collapse to icons on a phone), the PROJECTIONS toggle drops the model row down, one season picker that is a strip wide and a menu on a phone"
...
run_js_test test_la_chop         "Chopped leagues: the Chopping Block — safe % by simulation, chop/danger/safe bands, chopped teams read off finished weeks, the pane swap"
run_js_test test_la_bafl         "League Analyzer BAFL Mode: the BAFL app's matchup card — category math, blend, win probability, ticks, projected line, the pane in BAFL Mode"
...
run_js_test test_mobile_scale   "Mobile slider scale (compressed range, true-max typing) + the ⓘ info-popup system"
...
if [ -f "$DIR/test_flacco_split.py" ] && [ -f "$PYBUILD" ]; then
  echo "═══ Python tests ═══"
  for pyt in test_live_adv_metrics test_flacco_split test_bake test_coord test_afc_nfc test_hc_hist test_hc_tables test_role_parse test_wiki_table test_ecr_py test_ecr_extract test_otc_extract test_sharp_pull test_sos_pull test_roster_moves test_roster_truth test_sumer_pull test_ktc_pull test_seed_refresh test_seed_cache test_sidecar_guard test_ol_pipeline test_state_block test_cfb_classes test_combine_prospect test_tc_projections test_market_archive test_inseason_truncate test_inseason_snaps test_draft_sim test_draft_history test_draft_playbook test_draft_corpus test_manager_profile test_weekly_charts test_cfb_ol_unit test_tc_mcp test_ol_team_fallbacks test_tendencies test_team_success_rate test_coaching_charting; do
