...
if [ -f "$DIR/test_flacco_split.py" ] && [ -f "$PYBUILD" ]; then
  echo "═══ Python tests ═══"
  for pyt in test_live_adv_metrics test_flacco_split test_bake test_coord test_afc_nfc test_hc_hist test_role_parse test_wiki_table test_ecr_py test_ecr_extract test_otc_extract test_sharp_pull test_sos_pull test_roster_moves test_roster_truth test_sumer_pull test_ktc_pull test_seed_refresh test_seed_cache test_ol_pipeline test_state_block test_cfb_classes test_combine_prospect test_tc_projections test_market_archive test_inseason_truncate test_inseason_snaps test_draft_sim test_draft_history test_draft_playbook test_draft_corpus test_manager_profile test_weekly_charts test_cfb_ol_unit test_tc_mcp test_ol_team_fallbacks test_tendencies test_team_success_rate test_coaching_charting; do
