...
(async()=>{
  ...
  await app.handle('com.sengi.triplecrown://auth/callback?access_token=qa&refresh_token=qr');
  chk(calls.sessions[1] && calls.sessions[1].access_token==='qa' && calls.sessions[1].refresh_token==='qr', 'the tokens may ride in the query too (the return page moves them there for the Android intent URL)');
  ...
  chk(/com\.sengi\.triplecrown:\/\/auth\/callback/.test(page) && /location\.search/.test(page) && /location\.hash/.test(page) && /id="open"/.test(page) && /location\.href=a\.href/.test(page), 'native-auth.html forwards the query and hash to the app\'s scheme, by itself and by a button');
  chk(/intent:\/\/auth\/callback/.test(page) && /package=com\.sengi\.triplecrown/.test(page) && /S\.browser_fallback_url=/.test(page) && /fallback=1/.test(page) && /\[q,h\]\.filter\(Boolean\)\.join\('&'\)/.test(page), 'on Android the hop is an intent URL with the tokens moved into the query, and no app answering lands back here with a plain message');
