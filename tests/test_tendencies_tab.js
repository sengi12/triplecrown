// The Playbook's Tendencies tab: the team's numbers beside the league's, ranks among the
// teams that have them, the season picker, and honest empties.
const elStore={};
...
const app=new Function(code+`return { render:_schemeRenderTendencies, seasons:_schemeTendSeasons, norm:_schemeNormTab,
  setNflverse:(n)=>{NFLVERSE=n;}, setTeam:(t)=>{schemeTeam=t;}, setSeason:(s)=>{schemeTendSeason=s;}, setYear:(y)=>{TC_SEASON.year=y;} };`)();
...
console.log('=== the tab ===');
chk(app.norm('tendencies')==='tendencies' && app.norm('Tendencies')==='tendencies' && app.norm('zzz')==='playbook', 'the tab id normalises');
chk(JSON.stringify(app.seasons())==='["2026","2025"]', 'the seasons with tendencies, newest first');
let h=app.render({team:'DET', season:'2025'});
chk(/Tendencies · 2025/.test(h) && /scheme-tend-season active[^>]*>2025</.test(h) && /2026 · live/.test(h), 'the picked season leads, the live season is offered as a chip');
chk(/How guessable is the call/.test(h) && />72\.0%</.test(h) && /\+6\.0 pp/.test(h) && /1st of 2/.test(h), 'guessability: the team\'s rate, beyond the situation, ranked first of two');
chk(/3rd &amp; long<small>80<\/small>/.test(h) && />90%</.test(h) && />85%</.test(h) && />\+5\.0 pp</.test(h) && !/Red zone<small>0/.test(h), 'situations: team vs league with the sample and the gap; an empty situation is left out');
chk(/Streak lift/.test(h) && /Formation hold/.test(h) && /Play action/.test(h) && /Cold/.test(h) && /Motion rate/.test(h), 'sequencing, play action (after a run vs cold) and motion sections');
chk(/blitz habits/.test(h) && /After a blitz/.test(h) && /Stacked box vs run/.test(h) && /3rd &amp; long<small>80<\/small>/.test(h), 'the defence: blitz habits by situation, the streak, box counts');
chk(/2nd of 2/.test(h.slice(h.indexOf('Motion rate'))) && /2nd of 2/.test(h.slice(h.indexOf('Blitz rate'))), 'Detroit ranks second of two on motion and blitz rate (Seattle leads both)');
chk(/The Side Quest/.test(h) && /1000 offensive plays/.test(h) && /2 teams ranked/.test(h), 'credits the methods and states the samples');
app.setSeason('2026');
h=app.render({team:'DET', season:'2025'});
chk(/Tendencies · 2026/.test(h) && /1 teams ranked/.test(h), 'the season chip switches to the live season');
h=app.render({team:'KC', season:'2026'});
chk(/No tendencies for KC in 2026 yet/.test(h), 'a team without a block in the season says so');
