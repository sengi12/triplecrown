...
def main():
    ...
    check("pass map: every located attempt is a row [air, side, result, yac, yardline, qtr, receiver, formation, out-of-pocket, out-of-bounds] in play order",
          len(_g1.get("plays", [])) == 9 and _g1["plays"][0] == [25, 0, 2, 3, 50, 1, 0, 0, None, 0] and _g1["plays"][1] == [5, 1, 0, 0, 50, 1, 1, 0, None, 0])
    ...
    check("target_trees: every target is a row [air, side, result, yac, yardline, qtr, route, formation, out-of-pocket, out-of-bounds] in play order — no charting: route null, pbp formation, pocket unknown",
          w is not None and g1.get("wk") == 1 and g1.get("plays") == [[6, 2, 1, 10, 63, 1, None, 0, None, 0], [4, 0, 2, 41, 45, 4, None, 0, None, 0],
    ...
    nv._load_pbp = lambda season, cols=None: _tob
    _wob = nv.target_trees_weekly(2026, min_targets_game=1, min_targets_season=1)["players"]["test receiver"]["games"][0]["plays"]
    check("target_trees: a CATCH that ended out of bounds carries the 10th field; an incompletion that went out does not (there is no tail to bend)",
          [p[9] for p in _wob] == [1, 0, 0, 0] and len(_wob[0]) == 10)
    ...
    check("target_trees: FTN's formation and out-of-pocket ride the row when charted (U/S/P → 0/1/2); pbp's shotgun stands in for uncharted plays, pocket unknown",
          _g[0][7:9] == [0, 1] and _g[1][7:9] == [2, 0] and _g[2][7:9] == [1, None] and _g[3][7:9] == [1, None])
    ...
    check("target_trees: once the charting lands every row gains a 7th slot — an index into the routes legend",
          ...
          and w2["games"][1]["plays"][0][6] == 2 and len(p2[0]) == 10)
